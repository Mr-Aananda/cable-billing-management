    ...
    <div class="widget-body">
            ...
            <div class="px-0 border-0 card card-body rounded-0">
                ...
                <form action="{{ route('stock.index') }}" method="GET">
                    ...
                    <div class="row">
                        <div class="col-4">
                            <label for="select-beast" class="form-label">Select Product</label>
                            <select id="select-beast" placeholder="... Choose one ..." autocomplete="off" name="name">
                                <option value="">... Choose one ...</option>
                                @foreach ($products as $product )
                                    <option {{ request()->search ? request()->name == $product->name ? 'selected' : '' : '' }} value="{{$product->name}}">{{$product->name}}</option>
                                @endforeach
                            </select>
