@section('title', 'Edit supplier due manage')
        ...
        <div class="widget-body">
            ...
            <div class="d-flex align-items-center flex-wrap">
                <nav aria-label="breadcrumb">
                    {{ Breadcrumbs::render('supplier payment edit',$supplierDue) }}
                </nav>
    ...
    <div class="col-md-12">
        <div class="widget mt-3">
            ...
            <div class="widget-body">
                ...
                <form action="{{ route('supplier-due-manage.update', $supplierDue->id) }}" method="POST">
                      @csrf
                      @method('PATCH')
                       <div class="mb-3 row">
                                <div class="col-2">
                                ...
                                <div class="col-6">
                                    <input type="date" name="date" value="{{ old('date',$supplierDue->date->format('Y-m-d'))}}" class="form-control @error('date') is-invalid @enderror" id="date" required>
                            ...
                            <div class="mb-3 row">
                                ...
                                <div class="col-4">
                                    <div class="input-group">
                                        <select name="payment_type" class="form-select @error('payment_type') is-invalid @enderror" id="item-part" required>
                                                <option value="cash" {{ $supplierDue->payment_type == 'cash' ? 'selected' : '' }}>Cash</option>
                                        </select>

                                          <div class="col-8">
                                            <input type="number" name="amount" value="{{ old('amount',abs($supplierDue->amount)) }}" class="form-control" placeholder="0.00" id="amount" aria-describedby="button-addon-paid" required>
                                          </div>
                                        <!-- error -->
                                        @error('payment_type')
                                        <div class="invalid-feedback">
                                ...
                                <div class="col-2">
                                        <select name="balance_status" class="form-select" aria-label="Default select example">
                                            <option value="0"  {{$supplierDue->amount <= 0 ? 'selected':''}} >Payable</option>
                                            <option value="1" {{$supplierDue->amount > 0 ? 'selected':''}} >Recieveable</option>
                                        </select>
                                    </div>
                            </div>
                                ...
                                <div class="col-6">
                                    <select name="cash_id" class="form-select  @error('cash_id') is-invalid @enderror" id="cash-id" required>
                                        @foreach ($cashes as $cash)
                                            <option value="{{ $cash->id }}" {{ (old('cash_id', $supplierDue->cash_id) == $cash->id) ? 'selected' : '' }} >
                                                {{ $cash->cash_name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="mb-3 row">
                                ...
                                <div class="col-6">
                                    <div class="input-group">
                                        <input type="number" name="adjustment" value="{{ old('adjustment',$supplierDue->adjustment) }}" class="form-control @error('adjustment') is-invalid @enderror" placeholder="0.00" id="adjustment" aria-describedby="button-addon-adjustment">
                                    </div>
                            ...
                            <div class="mb-3 row">
                                ...
                                <div class="col-6">
                                    <textarea name="note" class="form-control @error('note') is-invalid @enderror" id="note" rows="2"
                                        placeholder="Optional">{{ old('note',$supplierDue->note)}}</textarea>
