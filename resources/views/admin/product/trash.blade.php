        ...
        <div class="widget-body">
            ...
            <div class="d-flex align-items-center flex-wrap">
                <nav aria-label="breadcrumb">
                    {{ Breadcrumbs::render('product trash') }}
                </nav>
                ...
                <div class="ms-md-auto ms-0">
                    ...
                    <a href="{{ route('product.trash') }}" class="btn btn-sm btn-primary" title="Reload page">
                        <x-icons.refresh/>
                    </a>

                    <a href="{{ route('product.index') }}" class="btn btn-sm btn-primary" title="Go back">
                        <x-icons.back/>
                    </a>
            ...
            <div class="widget-body">
                <form action="{{ route('product.trash') }}" method="POST">
                                ...
                                <tr>
                                    <th scope="row" class="p-0">
                                        <label class="p-2 d-block">
                                            <input type="checkbox" name="products[]" value="{{ $product->id }}" class="me-2">
                                    ...
                                    <td class="print-none text-end">
                                        <a href="{{route('product.restore',$product->id)}}" title="Restore" class="btn btn-sm btn-outline-success">
                                            <x-icons.restore/>
                                        </a>

                                        <a href="{{ route('product.permanentDelete', $product->id) }}" class="btn btn-sm btn-outline-danger" onclick="return confirm('Are you sure, want to delete this data permanently?')" title="Permanent delete" >
                                            <x-icons.trash/>
                                        </a>
                    ...
                    <!-- End Table ============================================== -->
                        <!-- submit button -->
                    <div class="mt-3">
                        <button class="btn btn-sm btn-success me-2" name="restore" value="1" onclick="return confirm('Do you want to restore all selected record(s)?')" {{ ($products->count() > 0) ? '' : 'disabled'}}>Restore all</button>

                        <button class="btn btn-sm  btn-danger" name="delete" value="1" onclick="return confirm('The selected record(s) will be delete permanently!')" {{ ($products->count() > 0) ? '' : 'disabled'}}>Permanently delete</button>
                    </div>
                    <!-- submit button end -->
                </form>
    ...
    @push('scripts')
        ...
        <script>
            ...
            let masterCheckbox = document.getElementById("check-all"),
                childCheckbox = document.querySelectorAll('[name="products[]"]');
